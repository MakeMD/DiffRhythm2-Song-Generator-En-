...
def set_all_seeds(seed):
    ...
    np.random.seed(seed)
    # 3. PyTorch CPU and GPU seed
    torch.manual_seed(seed)
    # 4. If using CUDA（GPU）
    if torch.cuda.is_available():
        torch.cuda.manual_seed(seed)
        torch.cuda.manual_seed_all(seed)  # many GPU condition
        # torch.backends.cudnn.deterministic = True  # Ensure the convolution result is determined
        # torch.backends.cudnn.benchmark = False     # Disable optimization (sacrificing speed for determinism)
...
class DiffRhythm2Node:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "lyrics": ("STRING", {"forceInput": True}, {"tooltip": "lyrics tags include：[start]\n[end]\n[intro]\n[verse]\n[chorus]\n[outro]\n[inst]\n[solo]\n[bridge]\n[hook]\n[break]\n[stop]\n[space]"}),
                "music_style_prompts": ("STRING", {"default": "", "multiline": True, "tooltip": "Musical style description"}),
                "maximum_song_length": ("INT", {"default": 210, "min": 10, "max": 500, "step": 5}, {"tooltip": "maximum_song_length, Unit: second"}),
                },
            "optional": {
                                "reference_music": ("AUDIO", {"tooltip": "generate reference music similar songs"}),
                "sampler": (["euler", "midpoint", "rk4","implicit_adams"], {"default": "euler"}),
                ...
                "unloading_model": ("BOOLEAN", {"default": True}, {"tooltip": "After generation unloading_model，to free up memory"}),
            },
        }
    ...
    def diffrhythmgen(
            self,
            music_style_prompts: str,
            lyrics: str,
            maximum_song_length: int,
            reference_music: dict = None,
            sampler: str = "euler",
            steps: int = 30,
            cfg: float = 2.0,
            seed: int = 0,
            unloading_model: bool = True):
        ...
        lyrics_token = parse_lyrics(lyrics, TOKENIZER)
        lyrics_token = torch.tensor(sum(lyrics_token, []), dtype=torch.long, device=self.device)

        # preprocess style prompt
        assert music_style_prompts.strip() != "" or reference_music is not None, "music_style_prompts或reference_music, Please provide at least one"

        if reference_music is not None:
            prompt_wav, sr = reference_music["waveform"].squeeze(0), reference_music["sample_rate"]
            ...
            with torch.no_grad():
                style_prompt_embed = MUQ(texts = [music_style_prompts]).to(self.device)
        ...
        with torch.inference_mode():
            latent = DFM.sample_block_cache(
                ...
                duration=int(maximum_song_length * 5),
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "DiffRhythm2Node": "DiffRhythm2 Song generation",
}
