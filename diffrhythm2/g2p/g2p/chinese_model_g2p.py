...
class PolyDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        # ✅ stack 一次性转成 np.ndarray
        batch_label_starts = np.stack(batch_label_starts, axis=0)
        ...
        batch_label_starts = torch.from_numpy(batch_label_starts).long()
        batch_labels = torch.from_numpy(batch_labels).long()
        batch_pmasks = torch.from_numpy(batch_pmasks).long()
